<div>
    <x-propio>
        ...
        {{-- todo FIN Buscador y boton de crear producto --}}
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <tbody>
                    @foreach ($productos as $item)
                        <tr
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <button
                                        ...
                                        type="button" wire:click="bajarStock({{ $item->id }})">
                                    ...
                                    <div>
                                        <input type="text" readonly id="first_product" @class([
                                            'bg-gray-50 text-center w-14 border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 py-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400  dark:focus:ring-blue-500 dark:focus:border-blue-500',
                                            'text-red-600 font-bold line-through' => $item->stock == 0,
                                            'text-orange-600 font-bold' => $item->stock >= 1 && $item->stock <= 5,
                                            'text-green-600 font-bold' => $item->stock >= 6 && $item->stock <= 20,
                                        ])
                                            value="{{ $item->stock }}" required />
                                    </div>
                                    <button
                                        class="inline-flex items-center justify-center h-6 w-6 p-1 ms-3 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                                        type="button" wire:click="subirStock({{ $item->id }})">
                                        <span class="sr-only">Quantity button</span>
                                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                            ...
                            <td class="px-6 py-4 font-semibold cursor-pointer text-gray-900 dark:text-white">
                                <div class="flex items-center"
                                    wire:click="cambiarDisponibilidadClick({{ $item->id }})">
                                    <div @class([
                                        'h-2.5 w-2.5 rounded-full me-2',
                                        'bg-green-500' => $item->estado == 'DISPONIBLE',
                                        'bg-red-500' => $item->estado == 'NO DISPONIBLE',
                                    ])></div> {{ $item->estado }}
                                </div>
                            </td>
                            <td class="px-6 py-4 font-semibold text-gray-900 dark:text-white">
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('category.edit', $item->id) }}" class="text-yellow-600 hover:text-yellow-700 mr-2">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <button wire:click="pedirConfirmacion({{ $item->id }})" type="button" class="text-red-600 hover:text-red-700">
            ...
            @else
        <div class="h-screen w-screen mb-30 bg-gray-100 flex items-center">
            <div class="container flex flex-col md:flex-row items-center justify-center px-5 text-gray-700">
                   <div class="max-w-md">
                      <div class="text-5xl font-dark font-bold">404 Not Found</div>
                    <p
                      class="text-2xl md:text-3xl font-light leading-normal"
                    >Lo sentimos pero no se ha encontrado ninguna categoria con ese nombre </p>
                    <p class="mb-8">Por favor verifica que la categoria exista o intenta con otro nombre</p>
                  
                  <a href="{{route('products.principal')}}" class="px-4 inline py-2 text-sm font-medium leading-5 shadow text-white transition-colors duration-150 border border-transparent rounded-lg focus:outline-none focus:shadow-outline-blue bg-blue-600 active:bg-blue-600 hover:bg-blue-700">Volver a la pagina principal</a>
            </div>
              <div class="max-w-lg">
