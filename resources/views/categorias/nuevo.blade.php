        ...
        <div class="mx-4 p-6 rounded-xl shadow-xl bg-gray-400 dark:text-gray-200 sm:w-1/2 sm:mx-auto">
            <form action="{{ route('category.store') }}" method="POST" class="sm:max-w-lg sm:mx-auto">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" value="{{ old('nombre') }}" name="nombre" id="nombre" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Nombre de la categoría">
                ...
                <div class="mb-6">
                    ...
                    <select name="tipo" id="tipo-categoria" onchange="mostrarSelectPadres(this.value)" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        ...
                        <option value="padre" @if(old('tipo') == 'padre') selected @endif>Categoría Padre</option>
                ...
                <div class="mb-6" id="padre-select-container" style="display: none;">
                    ...
                    <select name="category_padre_id" id="select-padres" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        ...
                        @foreach($categoriasPadres as $categoriaPadre)
                            <option value="{{ $categoriaPadre->id }}" {{old('category_padre_id') == $categoriaPadre->id ? 'selected' : ''}}>{{ $categoriaPadre->nombre }}</option>
                        @endforeach
                    </select>
                    <x-input-error for="category_padre_id"></x-input-error>
                </div>
                <div class="flex flex-row-reverse">
                    <button type="submit" class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm  sm:w-auto px-5 py-2.5 text-center">Crear Categoría</button>
